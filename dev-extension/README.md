...
```
https://<rancher>/k8s/clusters/local/api/v1/namespaces/magic-closet/services/http:magic-closet-dev-extension:8005/proxy/
```
...
```
  magic-closet extension (loaded in Rancher)
     |
     |  ensureDevExtension()  on plugin load
     v
  ConfigMap  magic-closet-dev-extension    <- the seed: this directory, baked in
  Deployment magic-closet-dev-extension    <- node:24, runs /seed/boot.sh
  Service    magic-closet-dev-extension    <- ClusterIP :8005
     ^
...
- The seed is generated, not hand-copied:
  `node rancher-extension/scripts/gen-dev-extension-seed.mjs` writes
  `pkg/magic-closet/dev-extension-seed.generated.ts`. **Run it after editing anything here.**
...
`pkg/magic-closet/dev-extension.ts`, and the proxy path the pod is told about is built from
...
```bash
kubectl -n magic-closet exec -it deploy/magic-closet-dev-extension \
...
- **First boot takes a few minutes.** It pulls `node:24`, installs dependencies and compiles
  ...
  `node_modules` live on the node under `/var/lib/rancher/magic-closet/dev-extension`, so
...
- **Re-seeding is additive.** On boot the pod writes only seeded files it does not already
  ...
  `/var/lib/rancher/magic-closet/dev-extension` directory on the node.
